...
class MambaConfig:
    ...
    bidirectional: bool = False # use bidirectional MambaBlock
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.x_proj_dt_rank = nn.Linear(config.d_inner, config.dt_rank,bias=False)
        self.x_proj_B = nn.Linear(config.d_inner, config.d_state,bias=False)
        self.x_proj_C = nn.Linear(config.d_inner, config.d_state,bias=False)
        ...
        self.x_proj_dt_proj = nn.Linear(config.d_inner, config.d_inner, bias=True)
    ...
    def ssm(self, x, z, b_branch=False): 
        ...
        if hasattr(self,'use_split'):
            if not hasattr(self,"weight_copyed"):
                self.weight_copyed=True
                self.x_proj_dt_rank.weight.data = self.x_proj.weight.data[:self.config.dt_rank,:]
                self.x_proj_B.weight.data = self.x_proj.weight.data[self.config.dt_rank:self.config.dt_rank+self.config.d_state,:]
                self.x_proj_C.weight.data = self.x_proj.weight.data[self.config.dt_rank+self.config.d_state:,:]
                # self.x_proj_dt_proj.weight.data = dt_proj.weight.data@self.x_proj_dt_rank.weight.data
                # self.x_proj_dt_proj.bias.data = dt_proj.bias.data
            delta = softplus(dt_proj(self.x_proj_dt_rank(x)))
            B = self.x_proj_B(x)
            C = self.x_proj_C(x)
