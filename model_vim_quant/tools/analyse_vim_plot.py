...
def main(args):
    ...
    if args.use_hadmard:
        ...
        def substitute_layers(model):
            if args.use_hadmard_R1:
                ...
                R1 = RotateModule(h1).weight.to(model.layers[0].mixer.out_proj.weight.data)
            ...
            if args.use_hadmard_R4 or args.use_hadmard_R5:
                h4 = random_hadamard_matrix(model.layers[0].mixer.config.d_state,device)
                R4 = RotateModule(h4).weight.to(model.layers[0].mixer.out_proj.weight.data)
                h5 = random_hadamard_matrix(model.layers[0].mixer.config.dt_rank,device)
                R5 = RotateModule(h5).weight.to(model.layers[0].mixer.out_proj.weight.data)
                ...
                for layer in model.layers:
                    ...
                    if args.use_hadmard_R5:
                        ...
                        layer.mixer.x_proj_dt_b.weight.data = R5@layer.mixer.x_proj_dt_b.weight.data
                        layer.mixer.dt_proj_b.weight.data = layer.mixer.dt_proj_b.weight.data@R5.T
    ...
    global input_name
    input_name = "fig100_after_hadamard"
