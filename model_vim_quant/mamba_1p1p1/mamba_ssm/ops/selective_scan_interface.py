...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        """
             xz: (batch, dim, seqlen)  (batch,768,197)
        """
        ...
        if xz.stride(-1) != 1:
            xz = xz.contiguous()  
        conv1d_weight = rearrange(conv1d_weight, "d 1 w -> d w") #(384,4)
        x, z = xz.chunk(2, dim=1)  #张量分块函数 (batch,768,197) => (batch,384,197)  (batch,384,197)
