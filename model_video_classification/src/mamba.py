...
class PatchEmbed(BaseModule):
    ...
    def forward(self, x):
        ...
        if isinstance(self.projection, nn.Conv3d):
            ...
            B,C,L,H,W = x.shape
            p1 = L//self.init_out_size[0]
            p2 = H//self.init_out_size[1]
            p3 = W//self.init_out_size[1]
            
            x = x.reshape(-1, C, L//p1, p1, H//p2, p2, W//p3, p3)
            x = x.permute(0, 2, 4, 6, 1, 3, 5, 7)
            x = x.reshape(B, -1, 3*p1*p2*p3)
            x = self.projection(x)
            x = x.reshape(-1, L//p1, H//p2, W//p3, x.shape[-1])  # BLHWC
            x = x.permute(0, 4, 1, 2, 3)  # BCLHW
        ...
        if not self.fused_add_norm:
            ...
            if skip:
                ...
                if hasattr(self,'smooth'):
                    hidden_states = hidden_states*self.smooth + self.drop_path(x)
                else:
    ...
    def forward(self, x):
        ...
        if self.update_interval:
            ...
            #     x,residual = layer(x,residual)
                ...
                if i in self.out_indices:
                    outs.append(self._format_output(x, patch_resolution))
        # torch.save(ORDER,"./work_dirs/order.pth")
        # torch.save(SHAPE,"./work_dirs/shape.pth")
        # torch.save(N_POS_DIM,"./work_dirs/n_pos_dim.pth")
        return outs[-1]
